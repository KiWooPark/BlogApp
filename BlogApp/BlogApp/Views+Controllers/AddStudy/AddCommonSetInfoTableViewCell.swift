...
class AddCommonSetInfoTableViewCell: UITableViewCell {
    ...
    func configData(indexPath: IndexPath) {
        ...
        case 3:
            subTitleLabel.text = viewModel?.startDate.value == nil ? "시작 날짜를 설정해주세요." : viewModel?.startDate.value?.convertStartDate() ?? ""
        case 4:
            subTitleLabel.text = viewModel?.finishDay.value == nil ? "마감 요일을 설정해주세요." : viewModel?.finishDay.value?.convertSetDayStr() ?? ""
        case 5:
    ...
    @IBAction func tapSelectOptionButton(_ sender: Any) {
        ...
        case 4:
            delegate?.showNextVC(option: .selectDay)
        case 5:
            delegate?.showNextVC(option: .selectFine)
        case 6:
            switch indexPath?.row {
            case 0:
                delegate?.showNextVC(option: .addMemeber)
            default:
                return
            }
        default:
